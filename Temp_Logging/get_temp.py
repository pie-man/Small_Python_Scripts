...
SENSORS = {
      'window'      : '28-000005ea107a',
      'radiator'    : '28-000005e9b365',
      'room'        : '28-000005eb8b82',
...
def print_temps_sqlite(dbname, zone):
    try:
       conn = sqlite3.connect(dbname)
       curs = conn.cursor()
    except:
...
def print_temps_mysql(zone=None):
    try: 
        conn=MySQLdb.connect("localhost", "temp_log", "M4rv1n", "temp_logs")
        curs=conn.cursor()
    except:
        ...
        return
    if zone is not None:
        sel_zone = " WHERE location LIKE '{0:s}'".format(zone)
        print_out = "{0:10s} {2:8s}  :  {3:4.2f}"
    else:
        sel_zone = ""
        print_out = "{0:10s} {2:8s}  :  {4:20s} -> {3:4.2f}"
    curs.execute("SELECT * FROM temp_readings" + sel_zone)
    results = curs.fetchall()
    if zone is not None:
        print("Results for {0:s}\n====================".format(zone))
    print_out = "{0:10s} {2:8s}  :  {4:20s} -> {3:4.2f}"
    for row in results:
        print(print_out.format( row[0].strftime("%Y:%m:%d"), *row ))
    conn.close()
